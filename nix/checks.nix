    ...
    in {
      ...
      checks = {
        ...
        korgo-ssh-client-containment = pkgs.runCommand "korgo-ssh-client-containment" { } ''
          ...
          grep -F -- '[[ "$uid" =~ ^[0-9]+$ && "$gid" =~ ^[0-9]+$ ]]' "$launcher"
          grep -F -- '[[ "$user_name" =~ ^[a-z_][a-z0-9_-]*$ ]]' "$launcher"
          grep -F -- '[[ "$group_name" =~ ^[a-z_][a-z0-9_-]*$ ]]' "$launcher"
          grep -F -- 'passwd_record="''${user_name}:x:''${uid}:''${gid}:Korgo SSH Client:/var/empty:/bin/false"' "$launcher"
          grep -F -- 'group_record="''${group_name}:x:''${gid}:"' "$launcher"
          grep -F -- 'exec {passwd_fd}<<<"$passwd_record"' "$launcher"
          grep -F -- 'exec {group_fd}<<<"$group_record"' "$launcher"
          grep -F -- '--perms 0555' "$launcher"
          grep -F -- '--dir /etc' "$launcher"
          grep -F -- '--ro-bind-data "$passwd_fd" /etc/passwd' "$launcher"
          grep -F -- '--ro-bind-data "$group_fd" /etc/group' "$launcher"
          ! grep -E -- '--(ro-)?bind(-try)?[[:space:]]+(/etc([/[:space:]]|$)|[^#]*[[:space:]]/etc([/[:space:]]|$))' "$launcher"
          ...
          grep -F -- '--setenv XDG_SESSION_TYPE wayland' "$launcher"
          grep -F -- '--setenv FONTCONFIG_FILE @fontconfig_file@' "$launcher"
          ! grep -F -- 'ELECTRON_OZONE_PLATFORM_HINT' "$launcher"
          ...
          grep -F -- 'current UID has no sandbox passwd entry' "$probe"
          grep -F -- 'current GID has no sandbox group entry' "$probe"
          grep -F -- 'sandbox passwd does not contain exactly one record' "$probe"
          grep -F -- 'sandbox group does not contain exactly one record' "$probe"
          grep -F -- 'sandbox passwd record is not minimal and exact' "$probe"
          grep -F -- 'sandbox group record is not minimal and exact' "$probe"
          grep -F -- 'synthetic /etc is writable' "$probe"
          ...
          grep -F -- '--perms 0555' "$no_dbus_smoke"
          grep -F -- '--dir /etc' "$no_dbus_smoke"
          grep -F -- '--ro-bind-data "$passwd_fd" /etc/passwd' "$no_dbus_smoke"
          grep -F -- '--ro-bind-data "$group_fd" /etc/group' "$no_dbus_smoke"
          grep -F -- '--setenv XDG_SESSION_TYPE x11' "$no_dbus_smoke"
          ! grep -F -- 'ELECTRON_OZONE_PLATFORM_HINT' "$no_dbus_smoke"
